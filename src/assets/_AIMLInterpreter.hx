...
public function AIMLInterpreter(botAttributesParam)
{
    var self = this;
    botAttributes = botAttributesParam;

    function loadAIMLFilesIntoArray(fileArray)
    {
        isAIMLFileLoadingStarted = true;
        var fileIndex = 0;
        public function readAIMLFile(file)
        {
...
//            return cb('Unsupported file');
    ...
    function findAnswerInLoadedAIMLFiles(clientInput, cb)
    {//check if all AIML files have been loaded. If not, call this method again after a delay
        ...
        {
            public function findAnswerInLoadedAIMLFilesWrapper(clientInput, cb)
            {
                return function()
                {
            ...
            Timer.delay(findAnswerInLoadedAIMLFilesWrapper(clientInput, cb), 1000);
        }
    };
...
public function cleanStringFormatCharacters(str)
{
...
public function cleanDom(childNodes)
{
...
public function findCorrectCategory(clientInput, domCategories)
{//indexOfSetTagAmountWithWildCard indicates how many sets with wildcard occur so that those sets store the correct wildcard value
    ...
    public function travereseThroughDomToFindMatchingPattern(categories)
    {
...
//If it does not match, continue the traversion through the AIML file
    ...
    public function checkForThatMatching(categoryChildNodes)
    {
...
//if no that tag was found, everything 'fits'
    ...
    public function resolveChildNodesInPatternNode(patternChildNodes)
    {
        var text = '';
    ...
    public function findFinalTextInTemplateNode(childNodesOfTemplate)
    {
        var text = '';
...
//final text -> text after special nodes (bot, get, set,...) were resolved
            ...
            if (childNodesOfTemplate[i].name == 'condition'){
                return resolveSpecialNodes(childNodesOfTemplate);
            }
            else
            if (childNodesOfTemplate[i].name == 'random'){
//if random node was found, its children are 'li' nodes.
                return resolveSpecialNodes(childNodesOfTemplate);
            }
            else
            if (childNodesOfTemplate[i].name == 'srai'){
...
//call findCorrectCategory again to find the category that belongs to the srai node
            ...
            if (childNodesOfTemplate[i].name == 'li'){
                return findFinalTextInTemplateNode(childNodesOfTemplate[i].children);
            }
            else
            if (childNodesOfTemplate[i].name == 'br'){
//br elements are used for putting '\n' into the text
                return resolveSpecialNodes(childNodesOfTemplate);
            }
            else
            if (childNodesOfTemplate[i].name == 'pattern'){
...
//make use of the functions of the special nodes - bot, set, get...
            ...
            if (childNodesOfTemplate[i].name == 'think'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'bot'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'set'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'get'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'sr'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'star'){
                text = resolveSpecialNodes(childNodesOfTemplate);
                return text;
            }
            else
            if (childNodesOfTemplate[i].name == 'that'){
...
//return that text
    ...
    public function resolveSpecialNodes(innerNodes)
    {
        var text = '';
...
//If this set tag is a think tag's child
            ...
            if (innerNodes[i].name == 'br'){
                text = text + '\n';
            }
            else
            if (innerNodes[i].name == 'think'){
                previousThinkTag=true;
                text = text + resolveSpecialNodes(innerNodes[i].children);
            }
            else
            if (innerNodes[i].name == 'sr'){
                var result;
...
//Get a random number and find the li tag chosen
            ...
            if (innerNodes[i].name == 'star'){
                text = text + lastWildCardValue;
            }
            else
            if (innerNodes[i].name == 'srai'){
//take pattern text of srai node to get answer of another category
...
//call findCorrectCategory again to find the category that belongs to the srai node
            ...
            if (innerNodes[i].name == 'condition') {
...
// condition tag specification: multi condition tag
                ...
                if (innerNodes[i].attributes.value != undefined)
                {
...
public function checkIfMessageMatchesPattern(userInput, patternText)
...
public function convertWildcardToRegex(text)
{
...
public function getWildCardValue(userInput, patternText)
